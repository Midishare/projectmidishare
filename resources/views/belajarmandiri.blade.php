                ...
                @foreach ($belajarmandiri as $item)
                    <div class="col-12 col-md-6 col-lg-4 mb-4 d-flex align-items-stretch">
                        <div class="card shadow-sm h-100 w-100 p-1">
                            <img class="card-img-top" src="{{ asset('storage/icon/' . $item->gambar) }}" alt="item image"
                                style="height: 200px; width: auto; object-fit: cover;">
                            <div class="card-body d-flex flex-row justify-content-between">
                                <h5 class="">
                                    <a href="{{ route('belajarmandiri.detailmandiri', ['id' => $item->id]) }}"
                                        class="news-title-link">{{ \Illuminate\Support\Str::limit($item->judul, 20, '...') }}</a>
                                </h5>
                            </div>
                            <p class="card-text ms-auto"><small
                                    class="text-muted">{{ \Carbon\Carbon::parse($item->published_at)->format('d M Y') }}</small>
                            </p>
                        </div>
                    </div>
                @endforeach
