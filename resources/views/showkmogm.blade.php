            ...
            <form id="deleteForm" method="POST" action="{{ route('knowledgeogm.deleteSelected') }}">
                ...
                <table class="table table-responsive table-bordered table-hover table-striped" style="margin-top: 2rem;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <input type="checkbox" name="selectedItems[]" value="{{ $repo->id }}" class="select-item">
                            </td>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $repo->judulrepoogm }}</td>
                            <td>
                                @if ($repo->gambarrepoogm)
                                <img src="{{ asset('storage/gambar/' . $repo->gambarrepoogm) }}" alt="Gambar Repository" style="max-width: 100px; max-height: 100px;">
                                @else
                            ...
                            <td>
                                ...
                                <a href="{{ asset('storage/dokumen/' . $repo->dokumenfilerepoogm) }}" target="_blank">Lihat PDF</a>
                                @elseif (in_array(pathinfo($repo->dokumenfilerepoogm, PATHINFO_EXTENSION), ['doc', 'docx']))
                                <a href="{{ asset('storage/dokumen/' . $repo->dokumenfilerepoogm) }}" target="_blank">Unduh DOC</a>
                                @elseif (in_array(pathinfo($repo->dokumenfilerepoogm, PATHINFO_EXTENSION), ['ppt', 'pptx']))
                                <a href="{{ asset('storage/dokumen/' . $repo->dokumenfilerepoogm) }}" target="_blank">Lihat PPT</a>
                                @else
                            ...
                            <td>
                                ...
                                <a href="{{ url('/deletekmogm/' . $repo->id) }}" class="btn btn-danger" style="color: azure">
