...
@section('content')
<div class="container">
    <h2 style="margin-top: 100px;">Admin Financial Literasi - Materi Dokumen</h2>
    ...
    <div class="col-md-6">
        <a href="{{ route('admin.finlit.materi.create') }}">
    ...
    <form id="bulkDeleteForm" method="POST" action="{{ route('admin.finlit.materi.bulkDelete') }}">
        @csrf
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><img src="{{ asset('storage/' . $document->image_path) }}" alt="Document Image" width="100"></td>
                    <td><a href="{{ $document->link }}" target="_blank">View Document</a></td>
                    <td>{{ $document->created_at->format('d-m-Y') }}</td>
                    <td>
                        <a href="{{ route('admin.finlit.materi.edit', $document->id) }}" class="btn btn-primary">Edit</a>
...
<script>
    ...
    window.location.href = "{{ route('admin.finlit.materi') }}?search=" + encodeURIComponent(keyword);
}
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "{{ route('admin.finlit.materi.destroy', '') }}/" + id;
            }
        });
    }
