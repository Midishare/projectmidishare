    ...
    <div class="container">
        ...
        <form action="{{ route('admin.ip.materi.update', $document->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if ($document->image)
                    <img src="{{ asset('storage/icon/' . $document->image) }}" alt="Current Image"
            ...
            <div class="mb-3">
                ...
                <input type="url" class="form-control" id="link" name="link"
                    value="{{ old('link', $document->link) }}" required>
                @error('link')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select name="category" class="form-control" id="category" required>
                    ...
                    <option value="Human Capital" {{ $document->category == 'Human Capital' ? 'selected' : '' }}>Human
                        Capital</option>
                    <option value="Corporate Audit" {{ $document->category == 'Corporate Audit' ? 'selected' : '' }}>
                    ...
                    <option value="Merchandising" {{ $document->category == 'Merchandising' ? 'selected' : '' }}>
                    ...
                    <option value="Property Development"
                        {{ $document->category == 'Property Development' ? 'selected' : '' }}>Property Development</option>
                    <option value="Service Quality" {{ $document->category == 'Service Quality' ? 'selected' : '' }}>
                        Service Quality</option>
                    <option value="Corporate Legal & Compliance"
                        {{ $document->category == 'Corporate Legal & Compliance' ? 'selected' : '' }}>Corporate Legal &
                        Compliance</option>
                </select>
            </div>
