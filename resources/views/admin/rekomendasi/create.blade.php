...
<div class="container" style="margin-top: 100px;">
    ...
    <form id="rekomendasiForm" action="{{ route('rekomendasi.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="user_id" id="user_id" class="form-control" required>
                @foreach ($users as $user)
                    <option value="{{ $user->id }}" {{ (isset($rekomendasi) && $rekomendasi->user_id == $user->id) ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <div id="rekomendasi" class="summernote" style="height: 200px;">{{ isset($rekomendasi) ? $rekomendasi->rekomendasi : '' }}</div>
...
<script>
    $(document).ready(function() {
        ...
        // AJAX untuk mengambil rekomendasi saat pengguna dipilih
        $('#user_id').change(function() {
            var userId = $(this).val();
            if (userId) {
                $.ajax({
                    url: '{{ route('rekomendasi.get') }}', // URL untuk mengambil rekomendasi
                    method: 'GET',
                    data: { user_id: userId },
                ...
                $('#rekomendasi').summernote('code', ''); // Kosongkan jika tidak ada pengguna yang dipilih
            }
        });
    });
</script>
